raw='''turn off 660,55 through 986,197
...
for r in raw:
    temp = []
    c1,c2,c3 = r[0].split(), r[1].split(), r[2]
    temp.append(int(c1[len(c1)-1]))
    temp.append(int(c2[0]))
    temp.append(int(c2[2]))
    temp.append(int(c3))
    command = r[0].split(",")[0].split()
    if command[0] == "toggle":
...
for i in range(len(commands)):
    start_x, start_y, stop_x, stop_y = coords[i][0], coords[i][1], coords[i][2]+1, coords[i][3]+1
    if commands[i] == 'off':
