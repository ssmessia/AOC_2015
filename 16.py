raw='''Sue 1: goldfish: 9, cars: 0, samoyeds: 9
...
for r in raw:
    r.insert(0,int(r[0].split(': ')[0].split()[1]))
    r[1] = r[1].split(': ')[1] +": "+ r[1].split(': ')[2]
...
for r in raw2:
    l = []
    for R in raw:
        if r in R: #exact match, add it
            l.append(R[0])
        else: #can't remember, add it too
            item = r.split(': ')[0]
            items = [x.split(': ')[0] for x in R[1:]]
            if item not in items:
                l.append(R[0])
    P.append(l)

for i in range(len(P)-1):
    P[i+1] = list(set(P[i]).intersection(set(P[i+1])))
print(P[len(P)-1])

def checkRow(r,R):
    item, quantity = r.split(': ')[0], int(r.split(': ')[1])
    items = [x.split(': ')[0] for x in R[1:]]
    quantities = [x.split(': ')[1] for x in R[1:]]
    greater_than = ['cats', 'trees']
    less_than = ['pomeranians','goldfish']
    if item in greater_than and item in items:
        if int(quantities[items.index(item)]) > quantity:
            return True
    elif item in less_than and item in items:
        if int(quantities[items.index(item)])< quantity:
            return True
    elif r in R:
        return True
...
for r in raw2:
    l = []
    for R in raw:
        if checkRow(r,R) == True: #matches new rules
            l.append(R[0])
        else: #can't remember, add it too
            item = r.split(': ')[0]
            items = [x.split(': ')[0] for x in R[1:]]
